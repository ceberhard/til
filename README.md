# Today I Learned

This is a set of references for technologies topics that I come across as I am working as a professional software developer.

## Bash

- [Grep Count of Instances in String](bash/grep_count_in_text.md)
- [Variable Exists](bash/variable_exists.md)

## C#

- [Entity Framework 6 - View SQL](c-sharp/entity_framework_6_view_sql.md)
- [Local Functions](c-sharp/local_functions.md)
- [Pattern Match Switch Syntax](c-sharp/pattern_switch_syntax.md)
- [Record Types](c-sharp/record_type.md)
- [Save Stream to File](c-sharp/save_stream_to_file.md)
- [Top-Level Statements](c-sharp/top_level_statements.md)
- [Variable Assignment in Declaration Pattern Match](c-sharp/declaration_pattern_matching.md)

## Cosmos

- [Connect via C#](cosmosdb/connect_via_csharp.md)

## Git

- [Branching and Merging](git/git_branching_merging.md)

## Postman

- [Set Variables in Environment](postman/set_variables_in_env.md)

## Powershell

- [Comments](powershell/comments.md)
- [Conditionals](powershell/conditionals.md)
- [Get Script Location](powershell/get_script_location.md)
- [Modules](powershell/modules.md)
- [Scope](powershell/variable_scope.md)

## Python

- [Folder Exists](python/folder_exists.md)
- [String Instances Count](python/str_count_method.md)
- [Using urllib.parse.urljoin Method](python/using_urljoin.md)

## SQL

- [Select Random Rows](sql/select_random_rows.md)
